...
#include "f_system_time.h" // corresponding header
...
ISR (TIMER1_COMPA_vect){
	OCR1A = fsl::os::system_time::get_instance().get_compare_match_value_only_call_by_ISR();
	
	++fsl::os::system_time::get_instance();
	
	if (fsl::os::system_time::get_instance().now_update_interrupt_handler != nullptr) fsl::os::system_time::get_instance().now_update_interrupt_handler();
}

namespace fsl {
	namespace os {

		system_time* volatile system_time::p_instance {nullptr};
		
		uint8_t system_time::min_compare_match_value {4};

		bool system_time::start(){
		...
		uint8_t system_time::try_to_set(const long double& osc_frequency_new, uint8_t log_precision_wish){
		...
		system_time::system_time(const long double& osc_frequency, uint8_t& log_precision, uint8_t& error_code, fsl::str::void_function handler) : now_update_interrupt_handler(handler) {
		...
		uint16_t system_time::get_compare_match_value_only_call_by_ISR(){
